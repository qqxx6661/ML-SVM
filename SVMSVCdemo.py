...
data = []
labels = []
with open("ATM/ATM12.txt") as file:
    for line in file:
...
joblib.dump(clf_linear, "model/model_linear_ATM12.m")
...
start = time.time()
clf_rbf = SVC().fit(X, y)
joblib.dump(clf_rbf, "model/model_rbf_ATM12.m")
end = time.time()
print("time:", end - start)

start = time.time()
clf_poly = SVC(kernel='poly', degree=3).fit(X, y)
joblib.dump(clf_poly, "model/model_poly_ATM12.m")
end = time.time()
print("time:", end - start)

start = time.time()
clf_sigmoid = SVC(kernel='sigmoid').fit(X, y)
joblib.dump(clf_sigmoid, "model/model_sigmoid_ATM12.m")
end = time.time()
print("time:", end - start)
